...
from mappings.sipef import DESPESA_CODIGO_TO_TIPO_FORNECEDOR, DESPESA_CODIGO_TO_CLASS_FORNECEDOR, COD_HISTORICO_TO_DOCTIPO
...
def infer_and_fill_columns(data_frame: pandas.DataFrame) -> pandas.DataFrame:
    ...
    data_frame["Class.Fornecedor"] = data_frame["DESPESA CÓDIGO"].apply(
        lambda x: DESPESA_CODIGO_TO_CLASS_FORNECEDOR.get(to_int_safe(x), "")
    )
